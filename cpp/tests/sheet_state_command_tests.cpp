...
int main() {
    ...
    try {
        ...
        check(std::abs(attached->session.calculated_boundaries().back().volume-attached_volume)<1e-5,"Derived attachment did not return to its authored state.");
        ...
        check(edited_calculated.back().calculation_errors.empty()&&std::abs(edited_calculated.back().volume-attached_volume)<1e-5,
            "Cold native regeneration lost an extrusion authored between sheet states.");
