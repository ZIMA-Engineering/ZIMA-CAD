...
int main(int argc,char** argv) {
    ...
    try {
        ...
        for(bool unfold:{true,false}) {
            ...
            auto* edited=new app::SheetStateDialog(committed,{{"a","A"},{"b","B"}},[&](auto value){++commits;committed=std::move(value);},&parent);
            edited->show();application.processEvents();
            check(edited->findChild<QCheckBox*>("sheetStateIndividual")->isChecked()&&edited->selecting()&&edited->selected()==std::vector<std::string>{"a"},"Editing did not restore manual selection mode.");
            edited->reject();application.processEvents();
