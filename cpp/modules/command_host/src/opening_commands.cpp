...
void Host::register_opening_commands() {
    dispatcher_.add({"opening.components",tr("List the visible components of an opening without calculation."),{{"container",true},{"document",false}},false},[this](const Json& args) {
        try {
            const auto id=args.value("document",workspace_.active_document_id());const auto* state=workspace_.open_part(id);
            const auto& feature=opening(state,args.at("container"));auto items=Json::array();
    ...
    dispatcher_.add({"opening.component.remove",tr("Remove an optional opening component through the shared Tree transaction."),{{"container",true},{"role",true},{"document",false}},true},[this](const Json& args) {
        ...
        try {
            ...
            const auto* state=workspace_.open_part(id);static_cast<void>(opening(state,container));
            ...
            auto result=opening_details(*state,opening(state,container));result["component"]=role;result["changed"]=changed;
            result["body_calculated"]=changed;
