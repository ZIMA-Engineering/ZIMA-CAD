...
std::vector<zima::kernel::BoxOperation> PartDocument::box_operations() const {
...
PartDocument PartDocument::load(
    ...
    const auto expected_operations = document.box_operations();
    for (std::size_t boundary_index = 0;
        ...
        if (loaded_boundaries[boundary_index].source_fingerprint !=
            zima::kernel::box_history_fingerprint(
