...
void reject(const char* code,const char* message){throw SectionOperationError(code,message);}
...
bool set_section_placement_reference(Workspace& live,const std::string& id,
    const std::string& section,std::size_t index,Ref source,bool derive_orientation) {
    if(index>4||source.owner_id.empty()||source.semantic_key.empty()||!std::isfinite(source.offset))
        reject("invalid_arguments","A Section reference requires a source, a slot from 0 to 4 and a finite offset.");
    if(section.empty())reject("section_not_found","The requested Section does not exist in this document.");
    const auto edit=prepare_section_edit(live,id,section);
    if(live.open_part(id)&&!source.instance_path.empty())
        reject("invalid_reference","A Part Section requires a local original reference.");
    if(source.instance_path.empty()&&(source.owner_id==section||source.owner_id==edit.initial.sketch.id||source.owner_id==edit.initial.container_origin.id))
        reject("invalid_reference","A Section cannot reference its own geometry.");
    const auto geometry=section_placement_geometry(live,id);
    ...
    if(!point&&!axis&&!edge&&!face)reject("reference_not_found","The original Section placement reference is unavailable.");
    ...
    auto value=edit.initial;std::vector<Ref> position,orientation(2);std::array<bool,3> empty_locks{};
    ...
    if(position.size()>3)reject("invalid_placement","The Section has too many position reference rows.");
    auto baseline=value.placement;baseline.references=combined(position,orientation,index);
    if(!document::resolve_placement(baseline,geometry))reject("invalid_placement","Existing Section placement references cannot be resolved.");
    ...
    if(!document::resolve_placement(value.placement,geometry))reject("invalid_reference","The proposed Section placement references cannot be resolved.");
    return commit_section(live,edit,std::move(value));
}
}
