...
void check_source(const document::PartDocument& doc,const std::string& target,const Ref& ref) {
    ...
    if(target_body&&source_body) {
        ...
        if(source>destination)reject("reference_not_available","A primitive reference must precede it in model history.");
    ...
    if(owner==graph.owners.end())reject("reference_not_available","A primitive reference must precede it in model history.");
    ...
    if(source==doc.history_order.end()||destination==doc.history_order.end()||source>=destination)
        reject("reference_not_available","A primitive reference must precede it in model history.");
}
}
bool set_primitive_reference(Workspace& live,const kernel::OcctKernel& kernel,const std::string& id,
    ...
    if(index>4||source.owner_id.empty()||source.semantic_key.empty()||!source.instance_path.empty()||!std::isfinite(source.offset))
        reject("invalid_arguments","A primitive reference requires a local source, a slot from 0 to 4 and a finite offset.");
    const auto* state=live.open_part(id);if(!state)reject("unsupported_document","Primitive operations require an open Part.");
    ...
    if(!primitive_definition(existing->feature_kind))reject("wrong_feature","This container is not a supported primitive.");
    ...
    if(!point&&!axis&&!edge&&!face)reject("reference_not_found","The original primitive placement reference is unavailable.");
    ...
    if(position.size()>3)reject("invalid_placement","The primitive has too many position reference rows.");
    auto baseline=value.placement;baseline.references=combined(position,orientation,index);
    if(!document::resolve_placement(baseline,geometry))reject("invalid_placement","Existing primitive placement references cannot be resolved.");
    ...
    if(!document::resolve_placement(value.placement,geometry))reject("invalid_reference","The proposed primitive placement references cannot be resolved.");
