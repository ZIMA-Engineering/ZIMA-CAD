...
inline ViewerMesh pattern_copy_mesh(ViewerMesh mesh,PatternRequest p,unsigned index,const std::string& owner={},bool occurrence=false) {
    ...
    const auto geometry=[&](auto& g){for(auto& v:g.vertices)v=pattern_point(v,p,index);for(auto& r:g.triangle_references)face(r);
        for(auto& e:g.edges){if(e.annotation)transform_annotation(*e.annotation,[&](auto v){return pattern_point(v,p,index);});for(auto& v:e.points)v=pattern_point(v,p,index);if(e.exact_spline)for(auto& v:e.exact_spline->poles)v=pattern_point(v,p,index);ref(e.reference);
