...
inline ViewerMesh mirrored_viewer_mesh(ViewerMesh mesh,MirrorPlane plane,const std::string& owner={}) {
    ...
    const auto geometry=[&](auto& value){
        ...
        for(auto& edge:value.edges){if(edge.annotation)transform_annotation(*edge.annotation,[&](auto v){point(v);return v;});for(auto& p:edge.points)point(p);if(edge.exact_spline)for(auto& p:edge.exact_spline->poles)point(p);reference(edge.reference);
