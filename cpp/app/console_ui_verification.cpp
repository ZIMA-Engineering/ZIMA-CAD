...
int verify_command_console(QApplication& application,AssemblyWorkspaceWindow& window,const std::filesystem::path& directory) {
    ...
    try {
        ...
        for (const bool in_assembly : {false,true}) {
            ...
            const auto edit=[&](){
                QTreeWidgetItem* item=nullptr;
                for(QTreeWidgetItemIterator it(model_tree);*it;++it)if((*it)->data(0,Qt::UserRole).toString().toStdString()==owner){item=*it;break;}
                check(item,"Sketch container missing from Tree");window.show_tree_item_properties(item);flush();
                for(auto* candidate:window.findChildren<QDialog*>())
                    if(candidate->isVisible())if(auto* result=dynamic_cast<SketchPropertiesDialog*>(candidate))return result;
            ...
            for(const bool commit:{false,true}){
                ...
                dialog->findChild<QDoubleSpinBox*>("sketchPlaneOffset")->setValue(6);
                dialog->findChild<QPushButton*>("containerOrientationFlipButton")->click();
                dialog->findChild<QPushButton*>("containerOrientationRotateButton")->click();
                check(get().at("plane_offset_mm")==0,"Pending Sketch Properties escaped into document");
