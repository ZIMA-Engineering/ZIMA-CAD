...
private:
    ...
    QSpinBox* opacity_{};
    QLabel* preview_{};
    std::function<void(QColor)> accepted_;
    ...
    viewer_->set_context_menu_callback(
        [this](const auto& candidate, const QPoint& global_position) {
            ...
            if (candidate.kind == zima::viewer::CandidateKind::Dimension &&
                ...
                const bool locked = existing->driving;
                QMenu menu(this);
...
void AssemblyWorkspaceWindow::start_construction_reference_selection(
    ...
    viewer_->set_candidate_filter([this, prefix, active_part, index,
            ...
            baseline_references, baseline_dof,
...
void AssemblyWorkspaceWindow::accept_construction_reference(
    ...
    if (!orientation_reference && !is_shortcut_point &&
...
void AssemblyWorkspaceWindow::show_sketch_properties(const std::string& sketch_id) {
        ...
        }, this);
    {
        ...
        dialog->set_preview_callback([this, sketch_offset_drag](
            ...
            viewer_->set_extent_manipulator(
                zima::viewer::ExtentManipulator{
                    "sketch_plane_offset", {},
                    resolved_sketch.resolved_origin,
...
void AssemblyWorkspaceWindow::end_sketch_dimension_drag() {
    ...
    sketch_drag_document_.reset();
    assembly_sketch_drag_document_.reset();
    // A simple click selects a dimension but also passes through the same
    // press/release gesture path as a drag.  Preserve the camera even when
    // no placement changed; otherwise this harmless selection refits/jumps
    // the View on release.
    preserve_view_on_refresh_ = true;
    refresh_scene();
}
...
void AssemblyWorkspaceWindow::finish_pending_linear_dimension(
    ...
    const auto point_reference_position = [&](const std::string& reference_id)
        -> std::optional<std::array<double, 2>> {
        if (const auto* point = sketch->find_point(reference_id)) {
            return std::array{point->x, point->y};
        }
    ...
    const auto first = point_reference_position(
        pending_point_dimension_first_id_);
    const auto second = point_reference_position(
        pending_point_dimension_second_id_);
    auto kind = zima::sketcher::DimensionKind::Distance;
    if (first && second) {
        kind = zima::sketcher::classify_linear_dimension(
        ...
        [this, sketch_id, edit_mode, creation_kind, active_target,
         segment_dimension_creation](
            ...
            if (!edit_mode && active_target) {
                ...
                if (point_tool) {
                    sketch_point_dimension_active_ = true;
                    pending_point_dimension_kind_ = creation_kind;
                    pending_point_dimension_first_id_.clear();
                    pending_point_dimension_vertex_id_.clear();
                } else if (line_tool) {
                    sketch_line_pair_dimension_active_ = true;
...
void AssemblyWorkspaceWindow::show_body_color_dialog() {
    if (properties_dialog_ != nullptr) return;
    auto* dialog = new BodyColorDialog(selected_body_color(),
        [this](QColor color) { apply_body_color(color); }, this);
    properties_dialog_ = dialog;
    connect(dialog, &QObject::destroyed, this, [this] {
        properties_dialog_ = nullptr;
    });
...
void AssemblyWorkspaceWindow::refresh_scene() {
        ...
        if constexpr (requires { document.history; }) {
            ...
            if (container != nullptr) {
                ...
                const auto local_vector = [&](zima::kernel::Vec3 value) {
                    const auto endpoint = local(value.x, value.y, value.z);
                    return zima::kernel::Vec3{endpoint.x - origin.x,
                        endpoint.y - origin.y, endpoint.z - origin.z};
                };
    ...
    const auto active_assembly_display = [this](
            const zima::assembly::AssemblyDocument& source) {
    ...
    const bool drawing_tool_active = sketch_point_active_ ||
        sketch_segment_active_ || sketch_rectangle_active_ ||
        sketch_polygon_active_ || sketch_trim_active_ || sketch_circle_active_ ||
        sketch_mirror_active_ || sketch_arc_active_ || sketch_ellipse_active_ ||
        sketch_elliptical_arc_active_ || sketch_bspline_active_ ||
        sketch_coincident_active_ || sketch_midpoint_active_ ||
        sketch_symmetric_active_ || sketch_concentric_active_ ||
    ...
    viewer_->set_sketch_box_selection(
        ...
        [this](std::vector<zima::viewer::ViewerCandidate> candidates,
            ...
            for (const auto& candidate : candidates) {
                ...
                selected_sketch_geometry_ids_.insert(
                    candidate.semantic_key.substr(separator + 1));
            }
            QTreeWidgetItemIterator iterator(tree_);
            while (*iterator != nullptr) {
    ...
    const auto commit = [this, candidate, guarded] {
        ...
        try {
            ...
            if (candidate.semantic_key.starts_with("parameter:") &&
                ...
                inline_placement_dialog->owns_reference_owner(
                    candidate.owner_id)) {
                guarded->hide();
                ...
                if (key.starts_with("placement:")) {
                    ...
                    if (key == "angle") {
                        ...
                        changed = true;
                            ...
                            container->feature_kind == FeatureKind::Chamfer) &&
                           key == "size") positive(container->edge_treatment.size);
            }
