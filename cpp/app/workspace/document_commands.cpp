...
void AssemblyWorkspaceWindow::refresh_drawing_tree() {
    ...
    for(const auto& sheet:document.sheets) {
        ...
        for(const auto& view:sheet.views) {
            ...
            for(const auto kind:{drawing::ModelAnnotationKind::Dimension,drawing::ModelAnnotationKind::Axis,drawing::ModelAnnotationKind::Construction,drawing::ModelAnnotationKind::Symbol}) {
                ...
                if(kind==drawing::ModelAnnotationKind::Dimension)for(const auto& dimension:sheet.dimensions) {
                    ...
                    if(dimension.kind==drawing::DrawingDimensionKind::Chain&&dimension.chain_group.empty()&&!dimension.chain_datum_only)for(std::size_t i=0;i<dimension.segments.size();++i) {
                        ...
                        auto* branch=new QTreeWidgetItem(child,{text});
                        branch->setData(0,Qt::UserRole,QString::fromStdString("drawing-dimension:"+dimension.id+":branch:"+segment.id));
