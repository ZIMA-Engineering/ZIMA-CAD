# ZIMA-CAD — frozen Python reference

This is the final archived Python implementation. It receives no new features
or bug fixes; active development continues exclusively in the repository's
C++ implementation.

From the repository root, run this archived version with:

```bash
./archive/python/run.sh -w Projects
```

The launcher intentionally uses the shared repository `config/`, `resources/`,
`Projects/` and `runtime/` directories so the archived source does not require
a duplicate runtime or user data tree.
