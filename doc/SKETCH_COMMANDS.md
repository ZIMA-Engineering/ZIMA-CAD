...
and `instance_path` identify the exact persisted source. With `profile=false`,
all references, including faces, come from original objects. With `profile=true`,
External Geometry projects the actual calculated body edge at the Sketch input
boundary, including edges trimmed or generated by Chamfer/Fillet. It retains
the original identity where inherited, but uses the surviving body geometry.
Sequential edge indices are never accepted as identities. The native
`body_edge` flag preserves this choice for the edge and its derived endpoints.
`sketch.reference.project` retains the existing reference's explicit source mode.
