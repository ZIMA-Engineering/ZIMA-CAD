...
## Aktuální stav — dokončeno pro současné CAD operace

K 2026-09-15 je příkazové pokrytí současných podporovaných funkcí uzavřené:
**296 příkazů**, společné modelové operace pro GUI a CLI a dokončený audit
zbývajících přímých zápisů. Obě aplikace i všechny testovací programy jsou
sestavené. Úplná Windows Release regrese prošla **161/161 za 675,84 s**.

Rozsah a samostatné navazující etapy vysvětluje
[CLI_GUI_AUDIT.md](CLI_GUI_AUDIT.md); použití procesu popisuje
[CAD_COMMAND_LINE.md](CAD_COMMAND_LINE.md).
Následující chronologické záznamy uchovávají také stav dřívějších etap.
...
| Oblast | Stav | Pokrytí a hranice rozsahu |
| --- | --- | --- |
...
| Tělesa a Boolean | Hotovo včetně společné viditelnosti a kurzorů | Tvorba, čtení, aktivace, název/viditelnost, kurzory, Boolean, pořadí a mazání; `body.reference.set`, společný vstup `placement.reference.set` a odebrání přes `placement.reference.remove` |
...
| Uložené pohledy | Hotovo | `view.named.list/get/set/delete`: úplná kamera Partu/Assembly, společné GUI potvrzení, nativní uložení a Undo/Redo |
...
## Čtení uložených řezů (2026-09-13)

`section.list/get/components` používají společné definice a pravidla
šrafování s GUI. Vrací identitu vlastněné skici, soustavy řezu a přesné
cesty výskytů. Poškozený řez i nastavení již chybějící komponenty zůstávají
čitelné. Čtení nemění historii, aktivaci ani vypočtenou geometrii.

Modelové regrese a katalog prošly **2/2 za 0,53 s**. Obě aplikace se sestavily.
Integrační sada prošla 4/5; po opravě převodu cesty v novém testovacím vstupu
prošel i skutečný CLI proces **1/1 za 21,32 s**. Produkční kód se již nezměnil;
...
## Atomické dávky řezové skici (2026-09-13)

`section.sketch.edit` používá původní typované deklarace a nativní mutace
skicáře nad jediným soukromým návrhem. Teprve platná konečná řezová čára
se potvrdí společnou transakcí vlastností. Zachovává identity, referenční
pravidla, přesné výskyty, jednu historii a vypočtená data těles. Chybný krok
vrací svůj index bez publikace mezivýsledků. Dokumentace:
[SECTION_COMMANDS.md](SECTION_COMMANDS.md).

Plná sada po sestavení obou aplikací prošla **125/126 za 534,86 s**; nová
GUI regrese odhalila neukončený příznak výběru referencí po zavření
vlastností řezu. Po lokální opravě OK/Cancel a novém sestavení prošla
...
## Zdrojové šrafování výkresů (2026-09-13)

`drawing.view.hatch.get/set` rozlišují aktuální styl zdrojového řezu
od skrytí šraf v jednom pohledu. GUI a CLI používají společnou validaci
a potvrzení; příkazy řezů sdílejí parser komponent. Režim `uncut` a styl
patří zdrojovému řezu, místní viditelnost zůstává ve výkresovém pohledu.
Kontrola identity brání záměně zdrojového souboru, kontrola platnosti
editace brání přepsání novějšího modelu. Zdroj se otevírá až při potvrzení
skutečné změny a automaticky se neukládá. Historie výkresu a zdroje
zůstávají samostatné. Podrobnosti: [DRAWING_HATCH_COMMANDS.md](DRAWING_HATCH_COMMANDS.md).

Obě aplikace i všechny testovací programy se sestavily. Nová modelová
sada prošla 1/1 za 0,32 s. První integrace 7/8 za 37,07 s odhalila
chybný předpoklad GUI testu o počtu desetinných míst; kontrola nyní
rozlišuje přesné uložené číslo od dvoumístného zobrazení. Navazující
scénář také správně obnovuje výběr pohledu po konzolové mutaci.
GUI a překlady poté prošly 2/2 za 16,82 s. Celková sada má 130 testů,
...
## Profilové odečty Assembly a úplné uložení parametrů (2026-09-13)

Příkazy `extrusion/revolution.create/get/set` používají ve vlastnící sestavě
stejnou transakci jako GUI. `assembly.cut.list` čte odečty bez výpočtu.
Konkrétní bezprostřední výskyty zůstávají oddělené, zdrojový Part se nemění;
chyba výpočtu ani neplatný cíl nezanechají částečnou regeneraci.

Modelová regrese odhalila neúplné ukládání parametrů sestavového profilu.
Part a Assembly nyní používají jednu profilovou serializaci, přesunutou
z velkého souboru PartDocument. Assembly INI má verzi 17, vnitřní verzi 26;
Part zůstává 19/43. Přípony zůstávají zachované, obě startovací šablony byly
přeuloženy a verzovaná fixture odpovídá současnému formátu. Podrobnosti
a průběžné ověření: [PROFILE_COMMANDS.md](PROFILE_COMMANDS.md).

Ověřen skutečný CLI proces i GUI editace. Cílená závěrečná regrese **4/4
za 13,93 s** a dodatečné kontroly chyb, vlastnictví a zámků **1/1 za 0,83 s**
prošly. Finální sestavení obou aplikací i všech testů následovala **úplná
regrese 136/136 za 540,06 s**, bez chyby (`build/assembly-profile-full-tests.log`).
Katalog má 236 příkazů. Následuje mazání, potlačení a pořadí sestavových
odečtů. Push čeká na návrat uživatele.


## Historie sestavových odečtů (2026-09-13)

`assembly.cut.suppress/remove/move/can_move` sdílejí transakce s kontextovou
nabídkou i přesunem ve stromu. Odstranění neuchovává osiřelou vlastněnou skicu;
promítnutá křivka jiného objektu zůstane se zřetelně porušeným místním odkazem.
Přesun z nabídky i tažením má stejné kontroly závislostí a původní geometrie.
Podrobnosti: [ASSEMBLY_CUT_HISTORY_COMMANDS.md](ASSEMBLY_CUT_HISTORY_COMMANDS.md).

Modely **3/3 za 1,58 s** a finální integrace **9/9 za 115,96 s** prošly.
Sestaveny obě aplikace a všechny testovací programy. Katalog má 240 příkazů,
...
## Editor šablon a atomická geometrie (2026-09-14)

`template.new/open/get/save/sketch.edit` zpřístupňují stávající editor `.frmz`
a `.tblz`. GUI a CLI sdílejí tvorbu, otevření a uložení; dávka geometrie
používá stejný skicový model, nativní písmo a jedinou transakci. Znovuotevření
respektuje neuložená data i různé cesty ke stejnému fyzickému souboru.
Samostatná kopie chrání otevřené cíle a vrací společný výsledek `paths`.

Konzole smí měnit klidovou šablonu; kreslicí nástroj, tažení a vlastnosti
blokují mutace. Výpočet tělesa se nevolá. Nový formát ani změna startovacích
modelových šablon nejsou potřeba. Podrobnosti: [TEMPLATE_COMMANDS.md](TEMPLATE_COMMANDS.md).

GUI regrese odhalila duplicitní větev nabídky geometrie ve stromu, která
předčasně zachytila text/B-spline a znemožnila jejich Vlastnosti. Zůstává jedna
úplná větev. Zavření nabídky bez výběru navíc nezkouší prázdnou akci změny
konstrukční geometrie. Text vytvořený příkazem se ověřuje přes tuto skutečnou
nabídku, Cancel, OK, Undo/Redo, Save, Copy a opětovné otevření.
...
## Reference profilů Partu (2026-09-14)

`extrusion.reference.set`, `revolution.reference.set` a rozšíření
`placement.reference.set` potvrzují původní reference existujícího profilu
přes společné `commit_profile`. Vlastní skica a těleso zůstávají jednou
transakcí; bezezměnový požadavek nepřepočítává. Společný řešič se nemění.
Podrobnosti: [PROFILE_REFERENCE_COMMANDS.md](PROFILE_REFERENCE_COMMANDS.md).

Sestaveny obě aplikace a všechny testovací cíle. Integrační sada prošla
**10/10 za 132,22 s**, včetně kontrol objemu a skutečného posunu tělesa,
CLI procesu, obou GUI Vlastností, Undo/Redo, překladů a nativního uložení.
...
Pokračuje audit zbývajících adaptérů umístění a souborových operací. Následně
schválená oprava roviny zobrazených kót umístění je dokončená, ověřená a pushnutá
v `a2ae66a`; tažení úchopu mění pouze rozložení kóty. Podpora tažení těchto
úchopů při současně otevřených Vlastnostech zůstává samostatnou nedokončenou
prací. Oprava zvětšování kót skic z `part02.prtz` je pushnutá v `6897d1d`.


## Reference umístění profilových odečtů Assembly (2026-09-14)

`extrusion.reference.set` a `revolution.reference.set` nyní používají také
stávající transakci profilových odečtů sestavy. Sdílená příprava s Partem
zachovává rozdělení polohy a FRONT/TOP, měření zamčené vzdálenosti, vlastní
skicu a přesné cesty výskytů. Přesun přípravy byl výslovně schválen uživatelem;
společný řešič ani kontrakt umístění se nemění. Zdrojový Part zůstává vlastníkem
své geometrie. Podrobnosti: [PROFILE_REFERENCE_COMMANDS.md](PROFILE_REFERENCE_COMMANDS.md).

Rozšířeny stávající modelové, procesní a GUI testy; katalog má stále
**276 příkazů**, sada **150 testů**. Další rozsah zahrnuje zbývající adaptéry
umístění (zejména tažení/import), práci s referencemi vložených drah a souborové
operace. Celé CLI tímto ještě není dokončené.
...
## Přímé hodnoty ve View a závěrečný audit (2026-09-15)

Katalogový rozměr závitu otvoru, offset komponentové vazby a rádius
3D křivky / vložené Sweep dráhy nyní potvrzují stejné operace jako
Vlastnosti a CLI. Neplatná hodnota zachová dokument, shodná hodnota
nepřidává krok historie. Rozpracované Vlastnosti nadále vlastní
své OK/Zrušit.

Nový test odhalil a vedl k opravě chybějící rádiusové kóty samostatné
3D křivky v Assembly. Stejná funkce nyní připravuje kótu pro Part
i aktivní Assembly, včetně přesného výskytu podsestavy v nadřazeném
dokumentu. Zobrazení používá uloženou křivku bez výpočtu těles.

GUI regrese porovná celé uložené soubory z obou vstupních cest,
ověří Undo, opakovanou stejnou hodnotu, neplatný rádius, meze a
zámek offsetu. U dvakrát vložené stejné podsestavy ověří právě jednu
kótu na aktivním výskytu a zápis do správného zdrojového dokumentu.
Rozšířená cílená sada prošla **1/1 za 148,19 s**.

Audit zbývajících přímých zápisů uzavřel mapování gest, publikování
náhledů a přechodů vlastněných profilů na existující modelové příkazy.
Nepřidává další příkazy simulující myš ani nové hodnotové manipulátory.
Katalog obsahuje **296 příkazů**, CTest **161 testů**; formáty a
startovní šablony se v této etapě nemění.

Podrobnosti: [INLINE_DIMENSION_COMMANDS.md](INLINE_DIMENSION_COMMANDS.md)
a [CLI_GUI_AUDIT.md](CLI_GUI_AUDIT.md).

Závěrečná úplná Windows Release regrese prošla **161/161 za 675,84 s**,
včetně modelových, procesových i GUI testů. Obě aplikace a všechny
testovací programy jsou sestavené. Příkazové pokrytí současných
podporovaných CAD operací je tímto uzavřené.

Ověřovací logy:
- `build/inline-shared-final-build.log`
- `build/inline-shared-final-console.log`
- `build/inline-shared-full-regression.log`
