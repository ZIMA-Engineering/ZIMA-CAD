...
and the context menu of a manually created linear or running dimension. Only
witness lines are offered: the dimension line, arrows and running spine are not
editable candidates. A jog starts on a witness line and the second click defines
the end of the bend and its lateral displacement. Break first selects a dimension
when launched from the toolbar, then takes two witness-line positions. Context
commands already own the selected dimension. The second point remains after the
first and before the next edit and witness endpoint. Esc discards pending input.

Selected dimensions expose purple grips for the two edit endpoints. Dragging
commits one undoable change on release; Esc restores the original presentation.
Intervals cannot overlap or cross the witness endpoints. Jogs shift the remaining
witness line and its attached dimension endpoint without changing the measured
value, source references or tolerances. A common running datum shares its witness
edits across its chain; branch edits belong only to the individual dimension.

The shared Dimension Properties window includes **Jogs and breaks** with the
standard progressive green-arrow row, removal crosses and inspection eyes.
Click an existing entry to reposition its second endpoint in the canvas. Changes
remain pending until OK, and Cancel restores the original dimension. Native
Drawing segment data stores edit IDs, witness side, normalized endpoint positions
and lateral paper-millimetre offsets. The same geometry drives canvas painting,
hover candidates, grips, PDF and DXF; no model calculation is performed.

Radius and diameter creation hover highlights the exact selected circle/arc
without an additional point marker on its perimeter.

Validation covers two-click input, rejection of dimension-line candidates,
purple-grip endpoint ordering, native save/reopen, overlap rejection, properties
preview/Cancel/OK, Undo, running branch isolation and shared-datum synchronization,
existing MMB+RMB navigation, five-language command labels and radial hover pixels.
The measurement, Drawing UI, translation and native measurement contracts pass;
the main Windows workspace startup check also passes. PDF/DXF and screenshot
proofs are generated by the measurement UI contract under `build/witness-edits-proof.*`.
