...
| P1 | Advanced Drawing parity | Fixed a real BOM-synchronization gap: C++ previously only populated a sheet's `bom_rows` once, on first view insertion, and never re-derived it afterwards, so adding/removing Assembly components left the BOM stale even after an explicit "Regenerovat pohled" (regenerate view) action. Extracted the existing inline BOM-building logic into a shared `build_bom_rows_for_source()` helper (used by both initial insertion and regeneration) and updated `DrawingWindow::regenerate_selected_view` to rebuild and overwrite `active_sheet()->bom_rows` from the view's source Assembly's *current* component list (only if the freshly-built list is non-empty, to tolerate a transient source-load failure without wiping existing rows). `--verify-startup` now scripts the full flow through the real window: inserts a Drawing view from the open inner Assembly (driving the real `DrawingSourceDialog` then `ViewPropertiesDialog`), verifies the BOM row's quantity reflects the Assembly's two Part occurrences, inserts a third occurrence into that Assembly through the real Insert-component menu action (switching tabs), switches back to the Drawing tab, selects the view (via a small test-only `DrawingWindow::select_view_for_test`/`DrawingCanvas::select_view_for_test` accessor, since production selection is normally driven by mouse-position hit-testing that a scripted test cannot reliably emulate), triggers regeneration, and verifies the BOM quantity updated to three. Also implemented rich title-block token resolution matching Python's `title_block.py`: added native `zima::drawing::resolve_title_block_text()`/`title_block_tokens()`/`title_block_token_scope()` (dotted `&token` chains, Czech-labelled parameter aliases via `parameter_aliases`, per-locale `parameter_values`, `document.file_stem`/`sheet.format`/`sheet.position`/`sheet.scale`/`bom.item_number`/`bom.quantity` system tokens, and `drawing.`/`local.` per-sheet local values), covered by contract tests ported directly from Python's `test_drawing_format.py` token-scope/resolution unit tests. Wired a `DrawingCanvas::set_title_block_context()` + `DrawingWindow::refresh_title_block_context()` that builds the context from the active sheet's primary view's source Part's `user_parameters`/`user_parameter_values`/`user_parameter_labels` (falls back to the previous raw hard-coded `&sheet.format`/`&sheet.scale` rendering when no context is available, e.g. an empty sheet) and repaints title-block fields with resolved text instead of the raw stored `value`. Verified stable over 15 combined `--verify-startup` runs with 0 failures and full ctest 9/9 still passing. Known remaining gaps versus Python (still open, deliberately out of scope for this fix): Assembly-sourced views do not yet contribute model-level tokens (only a Part source's `user_parameters` are read, matching the current C++ ownership model where Assemblies do not carry independent user parameters distinct from their components); embedded template identity — Python persists a `format_definition`/`title_block_definition` snapshot per sheet (`set_format_definition`/`set_title_block_definition` in `drawing.py`) so a saved Drawing keeps its exact frame/title-block geometry even if the on-disk `.frmz`/`.tblz` template file later changes, whereas C++'s `load_frame_template`/`load_title_block_template` only reload from the template file path with no persisted snapshot. Verified during this pass that several previously-assumed gaps are not actually gaps: Python's Drawing module has no angular/radius/diameter dimensions, notes, leaders or symbols either (`grep` of `drawing.py` confirms only linear/parallel-line dimensions exist), no auto-layout of views, and no editable BOM table UI (BOM rows are derived, not hand-edited, in both applications) — C++ already matches Python on all of these. C++ view repositioning by mouse drag (`drag_view_id_` in `DrawingCanvas`) already exists and roughly matches Python's `_drag_view_start` mechanism, though a side-by-side behavioural comparison (exact drag semantics, snapping) has not been scripted yet |
| P1 | File-management parity | Implemented the previously-dead C++ action stubs for rename, delete-current-file, delete-all-versions, delete-old-versions (with and without keep-latest) and working-directory-wide old-version cleanup (with and without keep-latest), matching the Python reference's confirmation-dialog and numeric-suffix (`document.ext.N`) archive conventions. Rename uses the shared in-application `RenameDocumentDialog` (a `PropertiesSubWindow`), validates/defaults the extension, rejects name collisions for both the main file and a companion Drawing (`.prtz`/`.asmz` → `.drwz`), rewrites Assembly component and Drawing view `source_path` references, then renames on disk and refreshes tabs. `--verify-startup` now scripts the full workflow through the real window and real `QMessageBox` dialogs (Yes clicked via `QTimer::singleShot` + `QApplication::activeModalWidget()`, since the dialogs are modal): creates versioned archive files, verifies delete-old-versions-keep-latest removes only the older archive, delete-old-versions removes all archives but keeps the current file, rename moves the file on disk, and working-directory-wide keep-latest cleans archives across multiple documents at once; verified stable over 80+ combined `--verify-startup` runs with 0 crashes and full ctest 9/9 still passing. Closed the previously-known gap versus Python's `_rename_document_file_to`: rename now also scans the renamed file's directory and the entire working directory (recursively) for Assembly (`.asmz`) and Drawing (`.drwz`) documents that are saved on disk but **not** currently open in the workspace, loads each one, rewrites any component/view `source_path` reference pointing at the renamed file, and re-saves it — mirroring Python's behaviour of loading every candidate document and rewriting references regardless of whether it happens to be open. `--verify-startup` covers this with a throwaway closed Assembly fixture (never opened in the workspace) whose component references the just-renamed Drawing, confirming the rename correctly updates that closed document's reference on disk; verified stable over 20+ combined `--verify-startup` runs with 0 crashes and full ctest 9/9 still passing |
