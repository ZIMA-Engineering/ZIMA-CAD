...
class DimensionPropertiesDialog(QDialog):
    ...
    def __init__(
        ...
        self.locked_check.setChecked(bool(style.get("locked", False)))
        form.addRow(
...
class MainWindow(QMainWindow):
    ...
    def _show_tree_context_menu(self, position: QPoint) -> None:
        ...
        if self._sketch_edit_entity_id is not None:
            if sketch_dimension_id is not None:
                ...
                if action == delete_dimension_action:
    ...
    def _show_native_viewer_context_menu(self, position: QPoint) -> None:
        if self.native_viewer.consume_context_menu_suppression():
            return
        # While a sketch is being edited, right-click belongs exclusively to
        # the sketcher's candidate cycling/cancel workflow. It must never
        # fall through to the generic object menu of the owning sketch.
        if self._sketch_edit_entity_id is not None:
            return
    ...
    def _handle_unified_dimension_reference_selection(
        ...
        if len(self._sketch_dimension_point_ids) > 1:
            # A selected segment represents its two endpoint nodes. An axis
            # instead expects exactly one point, so discard the complete
            # segment selection including its preview/highlight before
            # starting the new axis -> point operation.
            self._clear_unified_dimension_selection(sketch)
    ...
    def _on_sketch_dimension_cursor_moved(
        ...
        if len(self._sketch_dimension_point_ids) != 2:
            return
        first = points.get(self._sketch_dimension_point_ids[0])
        second = points.get(self._sketch_dimension_point_ids[1])
        frame = self._sketch_frame(sketch)
        ...
        cursor = (x, y)
        dimension_type = classify_linear_dimension(
            first_position,
            second_position,
            cursor,
        )
    ...
    def _commit_unified_dimension(self) -> bool:
        ...
        if (
            self._stored_sketch_model(sketch).dimension_dof_reduction(
                candidate
            )
            == 0
        ):
    ...
    def _handle_sketch_perpendicular_selection(
        ...
        reference_point_ids = {
            str(point_id)
            for point_id in reference.get("point_ids", ())
        }
        geometry_point_ids = {
            str(point_id)
            for point_id in geometry.get("point_ids", ())
        }
        if len(reference_point_ids & geometry_point_ids) != 1:
            self.statusBar().showMessage(
                tr("sketch.status.perpendicular.invalid_second")
            )
            return
        ...
        if any(
            ...
            and str(constraint.get("geometry_id", "")) == first_id
            for constraint in constraints
        ):
            self.statusBar().showMessage(
    ...
    def _show_sketch_points_dimensions(
        ...
        displayed_point_ids = {
            str(point.get("id", "")) for point in points
        }
        for point_index, point in enumerate(points):
            ...
            for coordinate in ("x", "y"):
                key = f"sketch_point:{point_id}:{coordinate}"
                if bool(
                    dimension_styles.get(key, {}).get("locked", False)
                ):
        ...
        for dimension_index, stored_dimension in enumerate(
            ...
            if (
                dimension_type
                not in ("distance", "distance_x", "distance_y")
                or not bool(stored_dimension.get("locked", True))
            ):
                continue
    ...
    def _show_dimension_context_menu(
        ...
        locked = (
            bool(self._dimension_styles(entity).get(key, {}).get("locked"))
        ...
        properties_action.triggered.connect(
            ...
            QTimer.singleShot(
                0,
                lambda: self._edit_dimension_properties(dimension_key),
            )
        )
    ...
    def _commit_dimension_value(self, key: str, raw_value: str) -> None:
        ...
        if (
            ...
            or (binding[0] == "sketch_distance" and value <= 0.0)
        ):
            self.statusBar().showMessage(
                tr("dimension.invalid_value", value=raw_value)
            )
